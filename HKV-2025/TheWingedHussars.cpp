...
namespace Polish
{
    int getPriority(LT::Entry& e)
    {
        ...
        case LEX_BITNOT: return 4;
        case LEX_STAR: case LEX_DIRSLASH: case LEX_PERSENT: return 3;
        default: return -1;
        }
    }

    bool TheWingedHussars(Lexer::LEX& tbls, Log::LOG& log)
    {
        ...
        for (int i = 0; i < tbls.lextable.size; i++)
        {
            if (curExprBegin < vpositions.size() && i == vpositions[curExprBegin])
            {
                ...
                if (lexcount > 1)
                {
                    bool rc = setTheWingedHussars(tbls.idtable, log, vpositions[curExprBegin], v);
                    if (!rc)
                        return false;
    ...
    bool __cdecl setTheWingedHussars(IT::IdTable& idtable, Log::LOG& log, int lextable_pos, ltvec& v)
    {
