		...
		if (lenta_position < lenta_size)
		{
			if (ISNS(st.top())) //извлекаем последний элемент стека и проверяем нетерминал ли он
			{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)//смотрим, если такое правило есть идём дальше
				{
					...
					//		bool restored = reststate(log);   // сначала откат
					//	savediagnois(NS_NORULECHAIN);      // теперь правильное правило

					//	rc = restored ? NS_NORULECHAIN : NS_NORULE;
					//};
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		case LENTA_END:
			MFST_TRACE4(log, "------>LENTA_END")
				...
				if (st.size() > 1) // Если стек не пуст (ошибка: код закончился неожиданно)
				{
					rc = false;
					lenta_position--; // Откатываемся на последний существующий символ для корректного номера строки

					bool isTempleBroken = false;
					// Пробегаем по стеку и ищем ']', который закрывает temple
					// st.c - это доступ к внутреннему контейнеру твоего стека (благодаря my_stack_SHORT)
					...
					*log.stream << "-------------------------------------------------------------------------------------" << std::endl;
					...
					if (isTempleBroken)
					{
						// Если нашли забытый ']', принудительно кидаем ошибку 600 (Падение Храма)
						sprintf_s(buf, MFST_DIAGN_MAXSIZE, "Ошибка 600: строка %d, %s", lex.lextable.table[lenta_size - 1].sn, Error::GetError(600).message);
						*log.stream << buf << std::endl;
					}
					else
					{
						// Иначе выводим ошибку того правила, на котором остановились (как было раньше)
